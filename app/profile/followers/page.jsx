...
export default function MyFollowersPage() {
  ...
  async function fetchFollowers() {
    try {
      const res = await fetch(`/api/user/${session.user.id}/followers`);
      if (res.ok) {
      ...
      console.error("Gagal load followers");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="sticky top-0 z-10 bg-[var(--bg)]/80 backdrop-blur-md border-b border-[var(--border)]">
        <div className="flex items-center gap-4 px-4 py-3">
          ...
          <h1 className="text-lg font-bold text-[var(--text)]">Pengikut</h1>
        </div>
      </div>
      ...
      <div>
        {followers.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-[var(--text-secondary)] text-[15px]">Belum ada pengikut</p>
          </div>
        ) : (
          followers.map((user) => (
